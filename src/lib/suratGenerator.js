...
async function generateSuratRekomendasi(rekomendasiEvent) {
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // ════════════════════════════════════════════
      //  KOP SURAT – persis template resmi Pengda Kalbar
      // ════════════════════════════════════════════
      // Logo pengda kalbar (kop surat)
      const logoCandidates = [
        path.join(__dirname, '..', '..', 'uploads', 'logo-kopsurat.png'),
        path.join(__dirname, '..', '..', '..', 'frontend', 'public', 'logo-kopsurat.png'),
        path.join(__dirname, '..', '..', 'uploads', 'logo-pengda-kalbar.png'),
        path.join(__dirname, '..', '..', '..', 'frontend', 'public', 'logo-pengda-kalbar.png'),
      ...
      doc.fontSize(12).font('Helvetica-Bold').fillColor(greenColor)
        .text('PROVINSI KALIMANTAN BARAT', textLeft, kopTop + 40, { width: textW, align: 'left' });
      ...
      const nomorSurat = `Nomor : ${rekomendasiEvent.nomorSurat || 'UM.     /FORBASI-KALBAR/     /' + year}`;
      ...
      doc.fontSize(fs10).font('Helvetica')
        .text('Berdasarkan hasil Verifikasi Faktual Forbasi dengan ini Pengurus Daerah Forbasi Provinsi Kalimantan Barat memberikan ', bodyIndent, y, {
      ...
      // Ketentuan 4
      doc.fontSize(fs10).font('Helvetica')
        .text('4.  Memberikan laporan kegiatan setelah penyelenggaraan selesai kepada Pengurus Cabang setempat dan Pengurus Daerah Forbasi Kalimantan Barat.', numIndent, y, { width: numW, lineGap: lg });
      y = doc.y + 20;
      ...
      y += 14;
      doc.fontSize(fs10).font('Helvetica-Bold')
        .text('PROVINSI KALIMANTAN BARAT', marginL, y, { width: cw, align: 'center' });
      y += 20;
